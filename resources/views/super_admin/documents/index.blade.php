...
<body class="has-navbar-vertical-aside navbar-vertical-aside-show-xl">
  ...
  <main id="content" role="main" class="main">
    ...
    <!-- Page Header -->
      <x-backend.index_page_header title1="Documents" :title="'document'" :user="'Super Admin'" :size="sizeof($documents)"/>
      <!-- End Page Header -->
        ...
        <div class="table-responsive datatable-custom">
          <table id="exportDatatable" class="js-datatable table table-borderless table-thead-bordered table-nowrap table-align-middle card-table"
            ...
            <tr>
              ...
              {{$document->code}}
              <td>
                <a role="button" class="btn btn-success btn-sm mr-3" href="{{url('print_document/'.$document->id)}}">
                    <i class="bi bi-printer me-1"></i> print
                  </a>
                <a role="button" class="btn btn-white btn-sm mr-3" href="{{url('document/edit/'.$document->id)}}">
                  <i class="bi-pencil-fill me-1"></i> Modifier
                </a>
                <a role="button" class="btn btn-danger btn-sm" href="#" onclick="if(confirm('Etes vous sur de vouloir suprimer le produit '+'{{$document->img_tag}}')){deleteProd('{{$document->id}}');}else{console.log('false');}">
  ...
  function deleteProd(id){
    window.location.href = "{{url('/')}}/document/delete?id="+id;
  }
  </script>
